...
const trainingImageCount = 10000;
const k = 10;
...
const getLabelAtIndex = (index, trainingOrTest) => {
  ...
  if (trainingOrTest == "training") {
    fd = fs.openSync('./data/train-labels-idx1-ubyte', 'r');
  }
  else if (trainingOrTest == "test") {
    fd = fs.openSync('./data/t10k-labels-idx1-ubyte', 'r');
  }
  else {
...
const getPixelArrayForImageAtIndex = (index, trainingOrTest, width = imageWidth, height = imageHeight) => {
  ...
  // open file
  if (trainingOrTest == "training") {
    fd = fs.openSync('./data/train-images-idx3-ubyte', 'r');
  }
  else if (trainingOrTest == "test") {
    fd = fs.openSync('./data/t10k-images-idx3-ubyte', 'r');
  }
  else {
    console.log("trainingOrTest argument not supplied correctly. should be supplied as either 'training' or 'test'. Using training as default");
    fd = fs.openSync('./data/train-images-idx3-ubyte', 'r');
...
app.post('/predictionFromImage', (req, res) => {

  // console.log("body:", req.body);
  const data = req.body.data;

  const imageAsTensor = tf.tensor(data);

  const prediction = predictFromImageTensor(imageAsTensor);
...
// fs.writeFile('samplePixelArray.txt', JSON.stringify(getPixelArrayForImageAtIndex(0, "test")), function (err) {
//   if (err) return console.log(err);
//   console.log('file written successfully');
// });

console.log(getLabelAtIndex(0, "test"));